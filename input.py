...
import cPickle
#import _pickle as cPickle # Python3 compatibility
import gzip
import math
import numpy as np
import os
...
def extract_mnist_data(filename, num_images, image_size, pixel_depth):
  ...
  if not tf.gfile.Exists(filename+".npy"):
    with gzip.open(filename) as bytestream:
      ...
      data = (data - (pixel_depth / 2.0)) / pixel_depth # How about using the CORRECT formula? data/255.0 - 0.5 # Why pixel_depth?
...
def ld_mnist(test_only=False):
  ...
  if test_only:
      test_data = extract_mnist_data(local_urls[0], 10000, 28, 1)
      test_labels = extract_mnist_labels(local_urls[1], 10000)
  else:
